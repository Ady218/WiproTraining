...
@given(u'buyer is on the OLX homepage') #step decorator
...
@when(u'buyer types product in searchbar') #step decorator
def step_impl(context):
...
@then(u'search results should be displayed') #step decorator
def step_impl(context):
    assert context.driver.title.__contains__("Cars") and context.driver.url__contains("cars"), 'Search Failed'
