...
# settings_text = driver.find_element(By.XPATH, '/html/body/div[2]/div[8]/div/div[2]/div[2]/span/span/g-popup/div[1]/div')
...
print(f'Child Example -> Found {len(rows)} in the first table.')
...
# above -> element located above

elmnt_above_password = driver.find_element(locate_with(By.TAG_NAME, 'input').above(password_field))
print(f"Above Example -> Text above password: {elmnt_above_password.get_attribute('placeholder')}")
elmnt_above_password.send_keys('standard_user')
time.sleep(5)

# below -> element located below

elmnt_below_username = driver.find_element(locate_with(By.TAG_NAME, 'input').below(username_field))
print(f"Below Example -> Placeholder below username: {elmnt_below_username.get_attribute('placeholder')}")
elmnt_below_username.send_keys('secret_sauce')
time.sleep(5)
login_button.click()
time.sleep(3)

# toRightof -> elemnt to the left of another
...
# near -> element close to another (within `50px default)
